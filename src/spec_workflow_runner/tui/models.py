"""State data models for TUI application.

This module defines all state data models including project state, spec state,
runner state, and application state.
"""
...
class AppState:
    ...
    task_list_visible: bool = False
    collapsed_projects: set[int] = field(default_factory=set)  # Indices of collapsed projects
    current_error: str | None = None
    status_message: str | None = None  # Info/success messages
    status_message_timestamp: datetime | None = None  # When status message was set
    last_key_pressed: str | None = None  # Track last key for debugging
    active_runners: dict[str, RunnerState] = field(default_factory=dict)

    # Default provider/model preferences
    default_provider: str = "codex"  # codex, claude, gemini
    default_model: str | None = None  # None means use provider's first model

    # Viewport state for scrolling
    tree_viewport_offset: int = 0  # First visible tree line
    tree_viewport_size: int = 20  # Number of tree lines visible
    task_scroll_offset: int = 0  # First visible task in task list

    # Performance optimization - track if re-render is needed
    _needs_render: bool = True  # Private flag, set to True when state changes

    def mark_dirty(self) -> None:
        """Mark state as changed, requiring re-render."""
        self._needs_render = True

    def mark_clean(self) -> None:
        """Mark state as clean after rendering."""
        self._needs_render = False

    @property
    def selected_project(self) -> ProjectState | None:
        """Get currently selected project."""
        if self.selected_project_index is not None and 0 <= self.selected_project_index < len(
            self.projects
        ):
